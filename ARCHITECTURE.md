...
Инструмент не падает клиентскими таймаутами: граф исполняется в фоновом потоке (`AskRunner`, src/rag_kb/ask_runner.py), а вызов ждёт до `ask_wait_seconds` (180 c). Успел — клиент получает готовый ответ; не успел — `status="in_progress"` и подсказка повторить тот же вопрос (повтор отдаёт результат из кеша мгновенно). Ошибки графа перехватываются и возвращаются как `status="error"` с текстом, а не MCP-ошибкой. Ответы последних `ask_cache_size` вопросов кешируются.
...
- **Режим грейдинга выбирается конфигом.** Поштучный (`per_chunk`, по умолчанию) — по вызову LLM на чанк: строже, чаще отклоняет мусор, и именно его «пустой» вердикт запускает корректирующий retry-цикл Corrective RAG; медленнее (top-k вызовов), но бюджет AskRunner (180 c) это покрывает. Пакетный (`batch`) — один вызов на все чанки («перечисли номера релевантных»): быстрее, но на 3B-модели оптимистичен — пропускает слаборелевантные фрагменты в генерацию и подавляет retry. Выбор — `RAGKB_GRADE_MODE`; парсер пакетного ответа строг к противоречиям («нет» вместе с номерами — фолбэк на поштучный).
...
| `ask_wait_seconds` | `RAGKB_ASK_WAIT_SECONDS` | `180` | Сколько ask_question ждёт ответ до отдачи in_progress |
