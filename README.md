...
Важно: индексация и ответы выполняются на CPU и занимают минуты (эмбеддинги всех чанков папки, несколько вызовов LLM на каждый вопрос) — для локальной установки без GPU это нормально. Поэтому `index_folder` запускает индексацию в фоне и отвечает мгновенно: следите за ходом через `index_status`, пока `indexing_in_progress` не станет `false`. Дальнейшая проверка — через инструменты ниже (например, `index_folder("./sample_docs")` и `ask_question`).
