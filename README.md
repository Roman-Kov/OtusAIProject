...
Важно: индексация и ответы выполняются на CPU и занимают время (эмбеддинги всех чанков папки, вызовы LLM на каждый вопрос) — для локальной установки без GPU это нормально. Модель удерживается в RAM (`keep_alive`), поэтому повторные вопросы отвечают быстро; первый вопрос после рестарта стека может занять заметное время (загрузка модели). Поэтому `index_folder` запускает индексацию в фоне и отвечает мгновенно: следите за ходом через `index_status`, пока `indexing_in_progress` не станет `false`. Дальнейшая проверка — через инструменты ниже (например, `index_folder("./sample_docs")` и `ask_question`).
